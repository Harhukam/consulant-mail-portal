...
$stmt = $user_read->runQuery("SELECT * FROM  tbl_admin WHERE adminID=:uid");
$stmt->execute(array(":uid"=>$_SESSION['adminSession']));
$row = $stmt->fetch(PDO::FETCH_ASSOC);


//use for count total messages
$stmt = $user_read->runQuery("SELECT count(*) FROM userdatafeed WHERE app_client_id = :abc AND app_admintrash='N' ");
$stmt->execute(array(":abc"=>$row['adminID']));
$countmails = $stmt->fetchColumn();

//show notification unread messages
$stmt = $user_read->runQuery("SELECT count(*) FROM userdatafeed WHERE app_client_id = :abc AND app_admintrash='N'  AND app_admin_read_status='unread-message' ");
$stmt->execute(array(":abc"=>$row['adminID']));
...
extract($urow);
    ...
    $change="read";
    $stmt = $user_read->runQuery("UPDATE userdatafeed SET app_admin_read_status=:c1 WHERE app_id=:chid");
...
?>
<a class="btn btn-green pull-right" href="download.php?file=<?php echo $urow['app_document'];?>" <i class="fa fa-paperclip"></i> Download Attachment</a>
<?php
}
?>
</div>
