...
$user_complete = new admin();

if(!$user_complete->is_logged_in())
{
$user_complete->redirect('index');
}

$stmt = $user_complete->runQuery("SELECT * FROM  tbl_admin WHERE adminID=:uid");
$stmt->execute(array(":uid"=>$_SESSION['adminSession']));
$row = $stmt->fetch(PDO::FETCH_ASSOC);


//use for count total messages
$stmt = $user_complete->runQuery("SELECT count(*) FROM userdatafeed WHERE app_client_id = :abc AND app_admintrash='N' ");
$stmt->execute(array(":abc"=>$row['adminID']));
$countmails = $stmt->fetchColumn();

//show notification unread messages
$stmt = $user_complete->runQuery("SELECT count(*) FROM userdatafeed WHERE app_client_id = :abc AND app_admintrash='N'  AND app_admin_read_status='unread-message' ");
$stmt->execute(array(":abc"=>$row['adminID']));
...
$stmt_delete = $user_complete->runQuery("DELETE FROM tbl_user WHERE userID =:userid");
...
<tbody>
<?php
$stmt = $user_complete->runQuery("SELECT * FROM tbl_user ORDER BY userID DESC");
$stmt->execute();
if($stmt->rowCount() > 0)
{
